...
{
    /// <summary>
    /// LAA, SCI, MID &amp; CMF Player by Philip Hassey philhassey@hotmail.com
    ...
    ///
    /// </summary>
    internal sealed class MidPlayer : IMusicPlayer
    {
        const int LUCAS_STYLE = 1;
        const int CMF_STYLE = 2;
        const int MIDI_STYLE = 4;
        const int SIERRA_STYLE = 8;

        // AdLib melodic and rhythm mode defines
        const int ADLIB_MELODIC = 0;
        const int ADLIB_RYTHM = 1;

        // File types
        const int FILE_LUCAS = 1;
        const int FILE_MIDI = 2;
        const int FILE_CMF = 3;
        const int FILE_SIERRA = 4;
        const int FILE_ADVSIERRA = 5;
        const int FILE_OLDLUCAS = 6;

        // AdLib standard operator table
        static readonly byte[] adlib_opadd = { 0x00, 0x01, 0x02, 0x08, 0x09, 0x0A, 0x10, 0x11, 0x12 };
        ...
        static readonly int[] fnums = { 0x16b, 0x181, 0x198, 0x1b0, 0x1ca, 0x1e5, 0x202, 0x220, 0x241, 0x263, 0x287, 0x2ae };
        ...
        string author, title, remarks, emptystr;
        long flen;
        long pos;
        long sierra_pos; //sierras gotta be special.. :>
        int subsongs;
        byte[] data;

        byte[] adlib_data = new byte[256];
        int adlib_style;
        int adlib_mode;
        byte[,] myinsbank = new byte[128, 16];
        ...
        public float RefreshRate => fwait > 0.01f ? fwait : 0.01f;

        public MidPlayer(IOpl opl)
        {
                                    ...
                                    if (ch[c].on != 0)
                                    {
                                        ...
                                        if (vel != 0 && ch[c].inum >= 0 && ch[c].inum < 128)
                                        {
                                            ...
                                            if ((adlib_style & MIDI_STYLE) != 0)
                                            {
                                                ...
                                                if (nv > 127) nv = 127;
                                                nv = MidiData.my_midi_fm_vol_table[nv];
                                ...
                                switch (ctrl)
                                {
                                    ...
                                    case 0x63:
                                        if ((adlib_style & CMF_STYLE) != 0)
                                        {
                                            ...
                                            //   2 == AM on
                                            //   3 == AM+VIB on
                                            midi_write_adlib(0xbd, (byte)((adlib_data[0xbd] & ~0xC0) | (vel << 6)));
                                    ...
                                    case 0x67:
                                        ...
                                        if ((adlib_style & CMF_STYLE) != 0)
                                        {
                                            adlib_mode = (int)vel;
                                            if (adlib_mode == ADLIB_RYTHM)
                                                midi_write_adlib(0xbd, (byte)(adlib_data[0xbd] | (1 << 5)));
                                            else
                                                midi_write_adlib(0xbd, (byte)(adlib_data[0xbd] & ~(1 << 5)));
                                        }
                                        break;
        ...
        private void midi_fm_playnote(int voice, int note, int volume)
        {
        ...
        private void midi_fm_volume(int voice, int volume)
        {
            int vol;
        ...
        private void midi_fm_percussion(int ch, byte[] inst)
        {
        ...
        private void midi_fm_instrument(int voice, byte[] inst)
        {
        ...
        private void midi_fm_endnote(int voice)
        {
        ...
        private void Rewind(int subsong)
        {
            ...
            for (i = 0; i < 128; i++)
                for (j = 0; j < 14; j++)
                    myinsbank[i, j] = MidiData.midi_fm_instruments[i, j];
            for (i = 0; i < 16; i++)
            {
                ch[i].inum = 0;
                ch[i].ins = new byte[11];
                for (j = 0; j < 11; j++)
            ...
            switch (type)
            {
                ...
                case FILE_CMF:
                    ...
                    i = getnexti(2);
                    // TODO: if (i != 0) title = (char*)data + i;
                    i = getnexti(2);
                    // TODO: if (i != 0) author = (char*)data + i;
                    i = getnexti(2);
                    // TODO: if (i != 0) remarks = (char*)data + i;
                ...
                case FILE_OLDLUCAS:
                    ...
                    break;
                case FILE_ADVSIERRA:
                    Array.Copy(smyinsbank, myinsbank, 128 * 16);
                    tins = stins;
                    deltas = 0x20;
        ...
        private void midi_write_adlib(int r, byte v)
        {
            Opl.WriteReg(r, v);
            adlib_data[r] = v;
        }
        ...
        private bool load_sierra_ins(string path)
        {
            var directory = Path.GetDirectoryName(path);
            var filename = Path.GetFileName(path);
            var patchPath = Path.Combine(directory, filename.Substring(0, 3) + "patch.003");
            if (!File.Exists(patchPath)) return false;

            using (var fs = File.OpenRead(patchPath))
            using (var br = new BinaryReader(fs))
            {
                byte[] ins = new byte[28];
                ...
                for (var i = 0; i < 2; i++)
                {
                    for (var k = 0; k < 48; k++)
                    {
                        ...
                        myinsbank[l, 2] = (byte)((ins[0] << 6) + ins[8]);
                        myinsbank[l, 3] = (byte)((ins[13] << 6) + ins[21]);

                        myinsbank[l, 4] = (byte)((ins[3] << 4) + ins[6]);
                        myinsbank[l, 5] = (byte)((ins[16] << 4) + ins[19]);
                        myinsbank[l, 6] = (byte)((ins[4] << 4) + ins[7]);
                        myinsbank[l, 7] = (byte)((ins[17] << 4) + ins[20]);

                        myinsbank[l, 8] = ins[26];
                        myinsbank[l, 9] = ins[27];

                        myinsbank[l, 10] = (byte)(((ins[2] << 1)) + (1 - (ins[12] & 1)));
                        //(ins[12] ? 0:1)+((ins[2]<<1));
            ...
            Array.Copy(myinsbank, smyinsbank, 128 * 16);
            return true;
        }
        ...
        private byte datalook(long pos)
        {
